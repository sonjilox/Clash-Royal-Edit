...
if (canvas) {
    canvas.onclick = (e) => {
        // Обязательно проверяем на null, так как индекс 0 (Рыцарь) — это не null
        ...
        // Берем ID карты из текущей "руки" (первые 4 карты в battleDeck)
        const cardId = battleDeck[selectedCardIdx];
        const card = DATABASE.find(c => c.id === cardId);

        if (!card) return;

        // Проверка: хватает ли эликсира и кликаем ли мы на свою половину (y > 385)
        if (elixir >= card.cost && (card.anyWhere || y > 385)) {
            elixir -= card.cost;

            // Создаем юнита (если есть count, создаем толпу, если нет — одного)
            const count = card.count || 1;
            for (let i = 0; i < count; i++) {
                const offsetX = (Math.random() - 0.5) * 50;
                const offsetY = (Math.random() - 0.5) * 50;
                units.push(new Unit(x + offsetX, y + offsetY, 'player', card));
            }

            // Ротация колоды: убираем карту из руки и кладем в конец очереди
            battleDeck.splice(selectedCardIdx, 1);
            battleDeck.push(cardId);

            selectedCardIdx = null; // Снимаем выделение
            renderBattleCards(); // Обновляем картинки карт
        }
    };
}
